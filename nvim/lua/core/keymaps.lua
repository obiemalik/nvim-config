...
-- Search Panel (grug-far)
map("n", "<leader>sp", ':lua require("grug-far").open()<CR>') -- Open
map(
    "n",
    "<leader>sw",
    ':lua require("grug-far").open({ prefills = { search = vim.fn.expand("<cword>") } })<CR>'
) -- Search Current Word
map("v", "<leader>sv", ':lua require("plugins.grug-far").open_visual()<CR>')
map(
    "n",
    "<leader>sf",
...
map("n", "<C-n>", function()
    ...
    local nvim_tree_was_open = ok and tree_api.tree.is_visible()
    if nvim_tree_was_open then
        vim.cmd("NvimTreeRefresh")
    end
    vim.cmd("NvimTreeToggle") -- open/close
end, { desc = "Refresh and Open" })
