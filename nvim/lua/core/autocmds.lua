            ...
            for i = 1, #diagnostic do
                if isUnderCursor == false then
                    if diagnostic[i].end_lnum ~= diagnostic[i].lnum then
                        ...
                        isUnderCursor = cursor[2] > diagnostic[i].col - 2
                            and cursor[2] < diagnostic[i].col + 2
                    else
                        if
                            math.abs(diagnostic[i].end_col - diagnostic[i].col)
                            < 2
                        then
                            isUnderCursor = cursor[2] > diagnostic[i].col - 2
                                and cursor[2] < diagnostic[i].end_col + 2
                        else
                            isUnderCursor = cursor[2] > diagnostic[i].col - 1
                                and cursor[2] < diagnostic[i].end_col
                        end
                    end
                end
            end
